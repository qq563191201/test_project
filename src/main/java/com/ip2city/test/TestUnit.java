...
import com.ip2city.DbMakerConfigException;
...
public class TestUnit {
    ...
    public static void main(String[] args) {
        try {
            DbSearcher _searcher = new DbSearcher(new DbConfig(), "f:/data/ip2region.db");
            BufferedReader bfr   = new BufferedReader(new FileReader("f:/data/ip.merge.txt"));
            BufferedWriter bwr   = new BufferedWriter(new FileWriter("f:/data/error_log.txt", true));
            int errCount  = 0;
            int lineCount = 0;
            String str = null;
